...
interface TriggerandAction {
     id: string; 
     name: string; 
     image: string;
     } 
interface TriggerResponse { 
    availableTriggers: TriggerandAction[]; 
}
interface ActionResponse { 
    availableActions:TriggerandAction[]

}

function useAvailableActionsAndTriggers() {
    const [availableActions, setAvailableActions] = useState<TriggerandAction[]>([]);
    const [availableTriggers, setAvailableTriggers] = useState<TriggerandAction[]>([]);

    useEffect(() => {
        axios.get<TriggerResponse >(`${BACKEND_URL}/api/v1/trigger/available`)
            .then(x => setAvailableTriggers(x.data.availableTriggers))

        axios.get<ActionResponse >(`${BACKEND_URL}/api/v1/actions/available`)
